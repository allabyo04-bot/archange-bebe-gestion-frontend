  ...
  async function imprimerListeParNom() {
    ...
    try {
      ...
      const html = `
        <html>
          <head>
            <title>${texte} — Archange Bébé</title>
            <meta charset="utf-8" />
            <style>
          ...
          <body>
            ...
            <h2>Liste des articles — Archange Bébé — imprimé le ${new Date().toLocaleDateString('fr-FR')}</h2>
            <table>
              <thead>
  ...
  function chargerDonnees() {
    ...
    Promise.all([appelApi('GET', '/articles'), appelApi('GET', '/familles')])
      .then(([listeArticles, listeFamilles]) => {
        ...
        // Arrivée depuis une autre page (ex: Stock → Inventaire) avec un article
        // précis à ouvrir directement, sans avoir à le rechercher ici.
        const idAOuvrir = searchParams.get('modifier');
        if (idAOuvrir) {
          const article = listeArticles.find((a) => String(a.id) === idAOuvrir);
          if (article) {
      ...
      {articleAffiche && (
        <div style={styles.superposition} onClick={() => setArticleAffiche(null)}>
          <div style={styles.panneauConsultation} onClick={(e) => e.stopPropagation()}>
            ...
            {(articleAffiche.photos?.[0]?.url || articleAffiche.photoUrl) && (
              <img
                src={articleAffiche.photos?.[0]?.url || articleAffiche.photoUrl}
                alt=""
        ...
        <div style={styles.overlay} onClick={() => setPanneauDeplacementOuvert(false)}>
          <div style={styles.panneauEtiquettes} onClick={(e) => e.stopPropagation()}>
            ...
            {nouvelleSousFamilleCibleOuverte && (
              <div style={styles.blocNouvelleSousFamille}>
                <input
                  style={styles.champInput}
                ...
                />
                <input
                  style={{ ...styles.champInput, marginTop: 6 }}
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
                  <button type="button" onClick={() => setNouvelleSousFamilleCibleOuverte(false)} style={styles.boutonAnnuler}>
                    Annuler
                  </button>
                  <button type="button" onClick={creerSousFamilleCible} style={styles.boutonValider}>
                    Créer cette sous-famille
                  </button>
                </div>
              </div>
            )}
...
function FormulaireArticle({ familles, articleEnEdition, onFermer, onFamillesMisesAJour, onCree, onModifie, onSyncArticle }) {
  ...
  const [stockDepots, setStockDepots] = useState(null);
  const [stockDepotsChargement, setStockDepotsChargement] = useState(false);
  ...
  function afficherStockDepots() {
    if (stockDepots) { setStockDepots(null); return; }
    setStockDepotsChargement(true);
    appelApi('GET', `/articles/${articleEnEdition.id}/stock`)
      .then(setStockDepots)
      .catch(() => setStockDepots([]))
      .finally(() => setStockDepotsChargement(false));
  }
  ...
  return (
    <div style={styles.overlay} onClick={onFermer}>
      <form style={styles.formulaire} onClick={(e) => e.stopPropagation()} onSubmit={gererSoumission}>
        ...
        {estEdition && (
          <div>
            <button type="button" onClick={afficherStockDepots} style={styles.lienStockDepots}>
              {stockDepots ? 'Masquer le stock par dépôt' : '📦 Voir le stock par dépôt'}
            </button>
            {stockDepotsChargement && <p style={{ fontSize: 12, color: 'var(--brown-soft)' }}>Chargement…</p>}
            {stockDepots && !stockDepotsChargement && (
              <div style={styles.blocStockDepots}>
                {stockDepots.length === 0 && (
                  <p style={{ fontSize: 12, color: 'var(--brown-soft)', margin: 0 }}>Aucun stock enregistré nulle part.</p>
                )}
                {stockDepots.map((s) => (
                  <div key={s.lieuId} style={styles.ligneStockDepot}>
                    <span>{s.lieuNom}</span>
...
const styles = {
  lienStockDepots: { border: 'none', background: 'transparent', color: 'var(--gold-deep)', cursor: 'pointer', fontWeight: 700, fontSize: 13, padding: 0 },
  blocStockDepots: { background: 'var(--cream)', borderRadius: 8, padding: '8px 12px', marginTop: 8, display: 'flex', flexDirection: 'column', gap: 6 },
  ligneStockDepot: { display: 'flex', justifyContent: 'space-between', fontSize: 13 },
  ...
  boutonEditer: { border: 'none', background: 'transparent', cursor: 'pointer', color: 'var(--error)', fontSize: 13 },
